defmodule PetsWeb.PostLive.Form do
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      ...
      <.form for={@form} id="post-form" phx-change="validate" phx-submit="save">
        ...
        <footer>
          <.button phx-disable-with="Agregando..." variant="primary">Agregar Post</.button>
          <.button navigate={return_path(@current_scope, @return_to, @post)}>Cancelar</.button>
        </footer>
      </.form>
    </Layouts.app>
    """
  ...
  defp apply_action(socket, :new, _params) do
    post = %Post{usuario_id: socket.assigns.current_scope.usuario.id}
    mascotas = Mascotas.list_mascotas_for_dropdown(socket.assigns.current_scope)
